...
module.exports = (app, connection) => {
    ...
    app.post(`${process.env.API_VERSION}/add-prompt`, async (req, res) => {
        try {
            ...
            if (!eventNameData) {
                ...
                if (existingData) {
                    if (!existingDataDesc) {
                        ...
                        const updateData = await promptsData.updatePromptLog(reqData.eventname, descriptionObj);
                    ...
                    const addData = await promptsData.addNew(reqData)
                    return res.status(200).send({
                        success: true,
